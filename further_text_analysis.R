...
decisao_keywords <- c("procedente", "improcedente", "maioria", "unânime", "unanimidade",
                      "mérito", "conheceu", "parcialmente", "modulação", "não conheceu",
                      "prejudicad*", "majoritária", "modulou")
plan2020 <- read_csv("~/R Projects/stf_text_mining/planilha2020_processada-dupl_clusters.csv")
plan2020_text <- readtext("~/R Projects/stf_text_mining/planilha2020_processada-dupl_clusters.csv", text_field = "decisao")  # Indicates the column with the full extent of the judicial opinion as the text field.
plan2020_text <- plan2020_text %>% drop_na(text)
...
tokens <- tokens_keep(tokens, pattern = decisao_keywords, valuetype = "regex")
dfm <- dfm(tokens, tolower = T, remove = stop, case_insensitive = T)
topfeatures(dfm, 10)   # Allows us to glimpse the 50 most common words of our dfm.
lda <- LDA(dfm, k = 3, control = list(seed = pi))  # Creates a LDA model with 6 topics.
doc_lda <- tidy(lda, matrix = "gamma")
terms(lda, 5)  # Shows 50 most common words of each topic.
...
gamma_table[, "main_topic"] <- apply(gamma_table[, 2:4], FUN = which.max, MARGIN = 1)  # Categorizes each opinion by dominant topic
...
write.xlsx(plan2020, 'planilha2020_processada_bytopic.xlsx', row.names = F)
